...
import time
import datetime
# Repository: https://github.com/Letscrypto25/Crypto-bot-3.git
...
async def execute_trade_alt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    user_data = get_user_data(user_id)
...
def execute_trade_alt(symbol, amount):
...
def get_user_tone(user_id):
    doc = db.collection("users").document(str(user_id)).get()
    if doc.exists:
        return doc.to_dict().get("tone", "analyst")  # Default to "analyst"
    return "not set"
...
async def edge_ai_emotion_check(user_id, trade_context):
    ...
    try:
        ...
        if not user_doc.exists:
            # If the document does not exist, return an appropriate error
            return {"error": "User not found"}
        
        # If the document exists, proceed with the logic
        ...
        # If there are any feedback messages, send them to the user
        if feedback_msgs:
            full_message = "\n".join(feedback_msgs)
...
if __name__ == "__main__":
    ...
    telegram_app.add_handler(CommandHandler("trade", execute_trade))
    telegram_app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, help_command))  # Default fallback
...
# --- Edge AI Emotion Check Integration ---
...
async def send_telegram_message(chat_id, text):
    ...
    async with aiohttp.ClientSession() as session:
        await session.post(TELEGRAM_API_URL, json={
            "chat_id": chat_id,
            "text": text
        })
...
async def edge_ai_emotion_check(user_id, trade_context):
    ...
    if len(recent_trades) >= 2:
        ...
        if last["type"] == "buy" and second_last["type"] == "sell":
            emotions.append("FOMO: You bought immediately after selling.")

        if last["type"] == "buy" and "result" in second_last and second_last["result"] == "loss":
            if last["amount"] > second_last["amount"] * 1.5:
                emotions.append("Revenge trading: Aggressive re-entry after a loss.")

        if abs(last["amount"] - second_last["amount"]) > second_last["amount"] * 1.5:
            emotions.append("Inconsistent trade sizing: Sudden large change in position size.")

    if len(recent_trades) >= 5:
        timestamps = [t["timestamp"] for t in recent_trades[-5:] if "timestamp" in t]
        if len(timestamps) == 5:
            now = datetime.now(timezone.utc)
            time_diffs = [(now - ts).total_seconds() for ts in timestamps if isinstance(ts, datetime)]
            if max(time_diffs) - min(time_diffs) < 3600:
                emotions.append("Overtrading: 5 trades within 1 hour.")

    if emotions:
        feedback = "⚠️ Emotional Trading Detected:
" + "
".join(f"- {e}" for e in emotions)
    else:
        feedback = "Your recent trades show discipline and consistency. Keep it up!"
