...
def is_registered(user_id):
    ...
    try:
        user_ref = db_root.child("users").child(str(user_id))
        user_data = user_ref.get()
        return user_data is not None
    except Exception as e:
        logger.error(f"Firebase read error in is_registered: {e}")
        return False

def register_user(user_id, user_info):
    """Register user info in Firebase DB."""
    try:
        user_ref = db_root.child("users").child(str(user_id))
        user_ref.set(user_info)
        logger.info(f"User {user_id} registered successfully.")
    except Exception as e:
        logger.error(f"Firebase write error in register_user: {e}")

# Command handler with chill vibe and registration check
def handle_command(text, user_id):
    if not text:
        return "Hey, I didn't catch that. Try /help for commands."

    text = text.strip().lower()

    # Register on /start if new user
    if text == "/start":
        if not is_registered(user_id):
            ...
            return "Welcome back! Ready to trade or just say hi? Use /help to see what I can do."

    # Require registration for other commands
    if not is_registered(user_id):
        return "Hey, you gotta start by sending /start to get access here."
    ...
    if text == "/help":
        return ("Here's what I can do:\n"
                ...
                "/help - Show this menu\n"
                "/status - See if I'm alive\n"
                "/trigger - Run the trading bot task\n"
                "Feel free to chill and ask for updates anytime.")

    if text == "/status":
        return "All good on my side. Smooth sailing here."

    if text == "/trigger":
        from tasks import run_auto_bot_task
        run_auto_bot_task.delay({"source": "manual_trigger", "user_id": user_id})
        return "Triggered the bot task. Sit tight, I'll keep you posted."

    return f"Sorry, I don't get '{text}' yet. Try /help or just chill."
